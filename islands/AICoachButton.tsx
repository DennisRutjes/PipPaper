...
export default function AICoachButton({ tradeId, existingAdvice, aiRating, aiGrade, aiProvider, aiTimestamp }: Props) {
    ...
    const handleEvaluate = async () => {
        ...
        try {
            // Attempt to capture chart image if available
            let chartImage = null;
            const canvas = document.getElementById("trade-chart-canvas") as HTMLCanvasElement;
            if (canvas) {
                try {
            ...
            const res = await fetch("/api/ai-coach", {
                ...
                body: JSON.stringify({ tradeId, chartImage }),
            });
